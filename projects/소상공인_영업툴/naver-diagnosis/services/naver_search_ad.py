        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    if keyword_list:
                        # 첫 번째 결과 (정확히 일치하는 키워드)
                        for item in keyword_list:
                            if item.get("relKeyword", "").lower() == keyword.lower():
                                return {
                                    "keyword": item.get("relKeyword", keyword),
                        ...
                        # 정확히 일치하는 게 없으면 첫 번째 결과 반환
                        first_item = keyword_list[0]
                        return {
                            "keyword": first_item.get("relKeyword", keyword),
                    ...
                    print(f"[NaverSearchAdAPI] API 오류: {response.status_code} - {response.text}")
                    return {
                        "keyword": keyword,
                        "monthly_search_pc": 0,
                        "monthly_search_mobile": 0,
